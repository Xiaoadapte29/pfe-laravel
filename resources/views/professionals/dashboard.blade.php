  ...
  <header class="bg-white shadow-sm border-b">
    <div class="container mx-auto px-6 py-6 flex justify-between items-center">
      <div class="flex items-center space-x-5">
        ...
        <div>
          <h1 class="text-3xl font-semibold text-gray-900">Bienvenue, {{ $professionalData['name'] }} !</h1>
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        @foreach ([
          ...
          <div class="bg-white p-6 rounded-lg shadow hover:shadow-lg transition">
            <p class="text-gray-500 font-semibold mb-2">{{ $stat['title'] }}</p>
          ...
          <div class="space-y-4 max-h-96 overflow-y-auto">
            @foreach($bookings as $booking)
              <div class="flex justify-between items-center bg-gray-50 rounded p-4 hover:bg-gray-100 transition">
                <div>
                  <p class="font-semibold text-gray-800">{{ $booking['client'] }}</p>
                  <p class="text-gray-600 text-sm">{{ $booking['service'] }}</p>
                </div>
                <div class="text-right">
                  <p class="font-semibold text-gray-900">${{ $booking['price'] }}</p>
        ...
        <div class="bg-white rounded-lg shadow p-6">
          ...
          <div>
            ...
            <div class="w-full bg-gray-200 rounded-full h-3 mb-6">
              <div class="bg-blue-600 h-3 rounded-full" style="width: {{ $professionalData['profileCompletion'] }}%"></div>
            </div>
        ...
        @forelse($reviews as $review)
          <article class="border-b pb-6 last:border-none">
            <div class="flex items-center space-x-4 mb-3">
              ...
              <div>
                <h3 class="font-semibold text-gray-900">{{ $review->client->name }}</h3>
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              @foreach($bookings as $booking)
                <tr>
                  <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $booking['client'] }}</td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $booking['service'] }}</td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $booking['date'] }} {{ $booking['time'] }}</td>
                 <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700 space-x-2">
  @if ($booking['status'] === 'pending')
    <form action="{{ route('bookings.acceptRefuse', ['booking' => $booking['id'], 'action' => 'accept']) }}" method="POST" class="inline">
      @csrf
      <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded transition">Accepter</button>
    </form>
    <form action="{{ route('bookings.acceptRefuse', ['booking' => $booking['id'], 'action' => 'refuse']) }}" method="POST" class="inline ml-2">
      @csrf
      <button type="submit" class="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded transition">Refuser</button>
    </form>
  @else
    <span class="capitalize font-semibold text-gray-700">{{ $booking['status'] }}</span>
  @endif
</td>

                  <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">${{ $booking['price'] }}</td>
                  <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700 space-x-2">
        ...
        <div class="px-6 py-4 border-t flex justify-between items-center text-sm text-gray-500">
          <div>
            Affichage de <span class="font-semibold">1</span> à <span class="font-semibold">{{ count($bookings) }}</span> sur <span class="font-semibold">{{ count($bookings) }}</span> résultats
          </div>
          <div class="space-x-2">
        ...
        <form class="space-y-4">
          ...
          <div>
            ...
            <select id="specialty" name="specialty" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-fixhome-primary focus:ring-fixhome-primary sm:text-sm">
              <option value="plumbing" {{ auth()->user()->specialty === 'plumbing' ? 'selected' : '' }}>Plumbing</option>
              <option value="electrical" {{ auth()->user()->specialty === 'electrical' ? 'selected' : '' }}>Electrical</option>
              <option value="carpentry" {{ auth()->user()->specialty === 'carpentry' ? 'selected' : '' }}>Carpentry</option>
              <option value="painting" {{ auth()->user()->specialty === 'painting' ? 'selected' : '' }}>Painting</option>
            </select>
          </div>
